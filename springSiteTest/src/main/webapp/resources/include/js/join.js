...
$(function() {
	...
	var message = [ "영문,숫자만 가능.6~12자로 입력해주세요", "영문,숫자,특수문자만 가능.8~15자 입력해주세요",
			"비밀번호와 비밀번호 확인란은 값이 일치해야합니다", "-포함 입력해주세요 예)010-0000-0000" ];
				...
				if (!formCheck($('#userId'), $('.error:eq(0)'), "아이디를"))
					...
					$.ajax({
						...
						success : function(resultData) {
							...
							if (resultData == "1") {
								$("#userId").parents(".form-group").find(
										".error").html("현재사용중인 아이디입니다");
							} else if (resultData == "2") {
								$("#userId").parents(".form-group").find(
										".error").html("사용가능한 아이디입니다");
								idConfirm = 2;
							}
						}
					});
